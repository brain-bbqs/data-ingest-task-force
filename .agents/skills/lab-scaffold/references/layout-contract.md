...
- Create output parent directories itself, and write into the DANDI layout the plan specified. When the standardized dandiset already holds earlier work, nest everything the pipeline writes under one subdirectory of it (sanes writes under `autogenerated/`) so the two never mix, and name that subdirectory in the driver rather than in the registered command.
