...
Data ingest repository for the Kemere lab.
...
containers/            The pinned, reproducible runtime (Dockerfile)
...
pyproject.toml        Repository tooling (ruff)
```
...
The declaration is intentionally *not* pinned. `containers/Dockerfile` resolves
it fresh at build time and the resulting image (by digest) is the
...
```bash
# Build locally...
docker build -t kemere-ingest -f containers/Dockerfile .
