...
1. `dandi download` the project's incoming dandiset (from the `dandi.emberarchive.org` instance) into `<ember-incoming>/<incoming_dandiset_id>/`. Runs inside `--dandi-image` (`docker pull` + `docker run`), not directly on the runner host.
...
4. `dandi upload` the standardized directory, also inside `--dandi-image`.
...
- `dandi` runs only inside `--dandi-image`, never on the bare runner host, so its credentials come entirely from a `DANDI_API_KEY` already present in dispatch's own environment (e.g. set by the calling workflow — see `data-ingest-runner`'s README) — dispatch forwards it into every container it starts by name only (`docker run -e DANDI_API_KEY`, no `=value`), never as a literal value on the argv. Verify against your `dandi` CLI version (baked into `--dandi-image`) whether a plain env var authenticates a *named* instance (`-i emberarchive`) the way you expect. This same key is also forwarded into a project's `container_image` when one's set, so a lab's conversion step can use it too, without the secret ever appearing in a logged command line;
