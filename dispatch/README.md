...
Note: Sanes's registration (incoming `000522`, standardized `000523`) runs `labs/sanes/code/batch_convert.py`, which converts every session folder found under the incoming dandiset in one invocation (skipping sessions whose output NWB already exists, unless dispatch appends `--overwrite`).
...
Output is written under an `autogenerated/` subdirectory of the standardized dandiset (`000523/autogenerated/sub-<subject>/...`), since that dandiset already holds earlier, hand-made work at its root.
...
Everything in `labs/sanes/code/config.yaml` is still marked PROVISIONAL. The values are the literals the original notebook was written with against one example dataset, and the conversion carries the rough edges listed in `labs/sanes/README.md`, so its output should not be treated as final yet.
