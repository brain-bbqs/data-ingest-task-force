...
Note: Sanes's registration (incoming `000522`, standardized `000523`) runs `labs/sanes/code/batch_convert.py`, which converts every session folder found under the incoming dandiset in one invocation (skipping sessions whose top-level multi-subject NWB already exists, unless dispatch appends `--overwrite`).
Its `script_path` deliberately stays pointed at `_sanes_to_nwb.py`, since that is where the conversion logic that determines output content lives.
Output is written under an `autogenerated/` subdirectory of the standardized dandiset (`000523/autogenerated/ses-<session>/...`, one per-track NWB plus the linking multi-subject NWB per session -- see `labs/sanes/README.md`), since that dandiset already holds earlier, hand-made work at its root.
A session is a folder under `sourcedata/raw/` holding `idx_*` chunk folders. Anything else there is reported and left alone, so the pre-existing contents of `000522` are not treated as input.
An empty incoming dandiset is fine: the batch driver reports that it found no session folders and exits 0, so the first pass converts nothing and succeeds rather than failing the run.

Sanes is the one project set to `upload_validation: ignore`. Its output is written under `autogenerated/`, so DANDI reports `NON_DANDI_FOLDERNAME` (it wants `sub-<id>/` at the dandiset root) and `dandi upload` would otherwise refuse the whole upload. The nesting is deliberate, since `000523` already holds earlier hand-made work at its root, so the upload is allowed through instead. The errors are still validated and logged on every run. Drop this back to the default once the layout question in [issue #20](https://github.com/brain-bbqs/data-ingest-task-force/issues/20) is settled.

Everything in `labs/sanes/code/config.yaml` is still marked PROVISIONAL. The values are the literals the v2 lab-supplied scripts were written with against one example dataset, and the conversion carries the rough edges listed in `labs/sanes/README.md`, so its output should not be treated as final yet.
...
Suthana's second project, `suthana/seeber-2024`, shares this same dandiset pair (see "Sibling projects" above): its raw and standardized data are nested under their own subdirectories rather than the dandiset root, so the two projects' data never mix.

`suthana/in-lab` is also set to `upload_validation: ignore`, for the same reason as Sanes: its output is nested under `in-lab_autogenerated/` (`labs/suthana/in-lab/code/batch_convert.py`'s `OUTPUT_SUBDIRECTORY`) because `000531` already holds earlier, hand-made work at its root, so every file reports `NON_DANDI_FOLDERNAME`. The errors are still validated and logged on every run.
