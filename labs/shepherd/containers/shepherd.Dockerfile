...
#
# NeuroDebian trixie (Debian 13) is the base layer, matching the sibling Kemere
# and Inman images and the DANDI-cache repositories. It provides Python 3.13,
# and from Debian main a recent FFmpeg. The converter shells out to `ffprobe`
# (part of FFmpeg) to read video duration and frame count -- that is this
...
LABEL org.opencontainers.image.description="Pinned runtime environment for the Shepherd-lab multicamera-behavior-to-NWB conversion pipeline (labs/shepherd/)."

# FFmpeg (provides ffprobe), Python, and the basics for cloning and TLS.
RUN apt-get update \
...
# The `test` extra adds pytest so this same image can also run the smoke test
# against mounted code.
